...
if (isset($_POST['add_payment'])) {
    ...
    if ($check_result->num_rows > 0) {
        ...
        $stmt = $conn->prepare("INSERT INTO payments (member_id, amount, receipt_no, payment_method, notes, is_student_discount, student_id, discount_amount) VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
        $stmt->bind_param("idsssisd", $member_id, $amount, $receipt_no, $payment_method, $notes, $is_student_discount, $student_id, $discount_amount);
        $stmt->execute();
        $stmt->close();
        $member_query->close();
