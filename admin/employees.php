...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    if (isset($_POST['add_employee'])) {
        ...
        // Check if on_duty column exists
        $result = $conn->query("SHOW COLUMNS FROM users LIKE 'on_duty'");
        if ($result->num_rows > 0) {
            $current_duty = $conn->query("SELECT on_duty FROM users WHERE id = $id")->fetch_assoc()['on_duty'];
            $new_duty = $current_duty ? 0 : 1;
            $conn->query("UPDATE users SET on_duty = $new_duty WHERE id = $id");
        } else {
            // Add the column if it doesn't exist
            $conn->query("ALTER TABLE users ADD COLUMN on_duty BOOLEAN DEFAULT FALSE");
            $conn->query("UPDATE users SET on_duty = 1 WHERE id = $id");
        }
        header("Location: employees.php");
        exit();
